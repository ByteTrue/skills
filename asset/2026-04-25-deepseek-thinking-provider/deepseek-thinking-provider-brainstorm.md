...
这次讨论前先扫了项目架构和现有代码：March 已经有 Provider 的 `Protocol × VendorPreset` 结构，DeepSeek preset 已存在；架构层也已经把 reasoning 作为跨 provider 的统一能力写进 `.bytetrue/architecture/reasoning.md`。真正缺口不在"有没有 DeepSeek 入口"，而在 DeepSeek 官方 Thinking Mode 的协议细节和 March 当前 wire 层之间还没对齐：
...
7. 现有 `.bytetrue/architecture/reasoning.md` 里 DeepSeek 描述偏旧，design 阶段需要同步更新架构文档，使其反映官方 Thinking Mode、`thinking` 开关、`reasoning_effort`、tool-call 回传约束。
