---
name: bt-roadmap
description: 把"大到塞不进单个 feature"的需求做成完整事前规划：概设 + 接口契约 + 子 feature 拆解清单，放在 `.bytetrue/roadmap/{slug}/`。两种模式 new / update。触发：用户说"我想要一个 X 系统"、"帮我把这块需求拆一下"、"开一份 roadmap"，或 feature-design 阶段发现需求太大。
---
...
`.bytetrue/roadmap/` 是项目的"规划层"——每个子目录承载一块大需求，主文档由三块构成：
...
三块**一起**作为这块大需求所有子 feature 的共同约束——每条子 feature 进 `bt-feat-design` 时，roadmap 第 2 块的接口契约就是它的**硬约束输入**（不能违反，要改先回 roadmap update）。

**为什么 roadmap 承载架构方案不放进 `architecture/`**：`bt-arch` 守"只记现状不记计划"。前瞻性架构方案属于"还没落地、可能还会变"的事前规划，放进 architecture 会污染那份系统地图。等子 feature 真正落地，对应接口由 `bt-feat-accept` 提炼回 `architecture/`——roadmap 完成过渡使命后归档。
...
> **读完本节后，先跳到「退出后」节看完收尾清单再回来写 roadmap**——退出后第 1 步就是 tracker 同步，你有义务一次性问完。

> 共享路径与命名约定看 `.bytetrue/reference/shared-conventions.md`。主文档和 items 完整模板看同目录 `reference.md`。
...
- `bt-brainstorm` 判为 case 3 移交过来（brainstorm 只做分诊，不做拆解）
- 已有 roadmap 加新子 feature / 改依赖 / 调顺序 / 标废弃
- feature-design 发现要做的事实际是多个 feature 集合，先退回拆

不适用：单 feature 能装下 → `bt-feat`；描述能力"是什么、边界" → `bt-req`；描述系统"结构怎么搭" → `bt-arch`；拍板长期规约 / 选型 → `bt-decide`。
...
**共同必读**：`.bytetrue/attention.md` + 用户素材 + `roadmap/` 其他 roadmap（防重复）+ `requirements/` 相关 req + `architecture/` 相关 doc。

**按情况读**：
- 相关 compound 沉淀：`python .bytetrue/tools/search-yaml.py --dir .bytetrue/compound --query "{大需求关键词}"`
- 已有相关 feature 方案
...
3. 每条子 feature 的 slug 规范？（grep `.bytetrue/features/` 确认不冲突）
...
**new**：建 `.bytetrue/roadmap/{slug}/`；写主文档（`status: active` / `created` / `last_reviewed` 当天）；写 items.yaml（每条 `status: planned`、`feature: null`）；`validate-yaml.py` 校验。
...
feature-design 发现接口契约不合理 / 漏了 / 描述不准 → **回 `bt-roadmap update` 改了再继续**，不要在 feature 里偷偷绕开（绕开会让下一条同模块 feature 接到老契约导致二次冲突）。

### acceptance 自动回写

`bt-feat-accept` 收尾时如果 design frontmatter 有 `roadmap` 字段就改对应 `roadmap_item` 的 `status: done`，同时同步主文档子 feature 清单勾选状态。职责在 `bt-feat-accept` 不在本技能。
...
| 方向 | 关系 |
|---|---|
| `bt-req` 配合 | req 记"为什么有这个能力"、roadmap 记"打算怎么分步做出来"。大需求下可能多份 req；缺 req 提示用户先 `bt-req` |
| `bt-arch` 配合 | architecture 记现状、roadmap 记若干步。读 arch 理解现状但不改它 |
| `bt-feat` 下游 | 每条子 feature 是未来一次 feature 流程的种子；起头时 design frontmatter 带 `roadmap` / `roadmap_item` |
| `bt-feat-accept` 回写方 | acceptance 自动改 items.yaml 为 `done`，本技能只定义格式不负责回写 |
| `bt-onboard` 创建者 | 建 `roadmap/` 空目录 |
| `bt-brainstorm` 上游 | case 3 移交本技能，带"真问题 / 大致范围 / 可能子模块"一句话汇总。本技能不重复分诊直接拆 |
