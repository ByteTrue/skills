  ...
  if (msg.cmd === 'management') {
    try {
      ...
      if (msg.method === 'reload') {
        chrome.alarms.create('bt-self-reload', { when: Date.now() + 200 });
        return { ok: true };
      }
...
function scheduleProbe() {
  ...
  chrome.alarms.create('bt-ws-probe', { delayInMinutes: 0.083 }); // ~5s
}

function scheduleKeepalive() {
  // Keep SW alive while WS is connected (~25s, under 30s SW timeout)
  chrome.alarms.create('bt-ws-keepalive', { delayInMinutes: 0.4 }); // ~24s
}
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === 'bt-self-reload') {
    chrome.runtime.reload();
    return;
  }
  if (alarm.name === 'bt-ws-keepalive') {
