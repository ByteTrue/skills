...
function analyzeNode(node, pPathType='main') {  
    // Handle non-element nodes and leaf nodes
    ...
    // Gather child-element information and sort it
    ...
    // Detect whether this is partitioning or overlay
  ...
  function handlePartitionContainer(childrenInfo, pathType) {  
    ...
    if (hasMainElement) {  
      ...
      for (let i = 1; i < childrenInfo.length; i++) {  
        ...
        if (child.node.textContent.trim().length > 200) isSecondary = true;  // P3: keep nodes with substantial text content
  ...
  function handleOverlayContainer(childrenInfo, pathType) {  
    // elementFromPoint ground truth: let the browser tell us who is visually on top
...
js_findMainList = r"""function findMainList(startElement = null) {
        ...
        // Global scan: collect candidate containers and sort by l1 + l2*0.1, where l2 is grandchild count to capture multi-layer structures such as tables
        const candidates = [];
        ...
        // Find candidate groups and score them for each container
        let allCandidates = [];
        ...
        // Deduplicate: remove results that overlap more than 50% with a higher-scoring candidate
        const kept = [];
    ...
    function scoreContainer(container, items) {
        ...
        // Return a low score when too few items are visible
        if (visibleItems < 3) return 0;
        // Guard against outliers: ensure areas do not exceed the container
        totalItemArea = Math.min(totalItemArea, containerArea * 0.98);
        const areaRatio = totalItemArea / containerArea;
        // 3. Compute each score using linear interpolation rather than steps
        // 3.2 Area-ratio score, up to 40 points, continuous curve
        // Use a sigmoid to smooth the scoring
        const areaScore = 40 / (1 + Math.exp(-12 * (areaRatio - 0.4)));

        // 3.3 Uniformity score, up to 20 points, continuous curve
        ...
        if (itemAreas.length >= 3) {
            ...
            // Exponential decay, the smaller the cv, the higher the score
        ...
        // 3.4 Container-size score, up to 15 points, continuous curve
        const viewportArea = window.innerWidth * window.innerHeight;
        ...
        let layoutScore = 0;
        if (items.length >= 3) {
            // Group coordinates and count rows and columns
            ...
            // If it is a single row or single column, give full score; otherwise evaluate grid quality
        ...
        // Total score, still keeping the overall score around 100
...
def smart_truncate(soup, budget, _depth=0):
    ...
    # Print the whole allocation plan first
    actions = []
    for i in tops:
        c, l = kids[i]
