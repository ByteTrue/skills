...
class Session:
    ...
    def mark_disconnected(self):
        if self.is_active():
            _log(f"Tab disconnected: {self.url} (Session: {self.id})")
...
class TMWebDriver:
    ...
    def start_http_server(self):
        ...
        def link():
            ...
            if data.get("cmd") == "find_session":
                ...
                return json.dumps(
                    {"r": self.find_session(url_pattern)}, ensure_ascii=False
                )
            if data.get("cmd") == "execute_js":
                ...
                try:
                    result = self.execute_js(
                        code, timeout=timeout, session_id=session_id
                    )
                    _log(
                        "[remote result]",
                        (str(code)[:50] + " RESULT:" + str(result)[:50]).replace(
                            "\n", " "
                        ),
                    )
    ...
    def _register_client(
        self, session_id: str, client: WebSocket, session_info
    ) -> None:
        ...
        if not session or not session.is_active():
            ...
            if not session or not session.is_active():
                ...
                if alive_sessions:
                    ...
                    _log(
                        f"Requested session {session_id} is disconnected; automatically switching to the latest active session: {session.id}"
                    )
        ...
        while exec_id not in self.results:
            ...
            if time.time() - start_time > timeout:
                if tp in ["ws", "ext_ws"]:
                    if hasjump:
                        return {
                            "result": f"Session {session_id} reloaded and new page is loading...",
                            "closed": 1,
                        }
                    if acked:
                        return {
                            "result": f"No response data in {timeout}s (ACK received, script may still be running)"
                        }
                    return {
                        "result": f"No response data in {timeout}s (no ACK, script may not have been delivered)"
                    }
                elif tp == "http":
                    if acked:
                        return {
                            "result": f"Session {session_id} no response in {timeout}s (delivered but no result)"
                        }
                    return {
                        "result": f"Session {session_id} no response in {timeout}s (script not polled)"
                    }
    ...
    def _remote_cmd(self, cmd):
        try:
            return requests.post(
                self.remote, headers={"Content-Type": "application/json"}, json=cmd
            ).json()
        except (ConnectionError, requests.exceptions.ConnectionError):
            raise ConnectionError(
                "CS Browser master is not running; see bt-browser_sop to start the master"
            )
    ...
    def set_session(self, url_pattern: str) -> bool:
        ...
        if len(matched) > 1:
            _log(
                f"Warning: found multiple sessions with URLs containing '{url_pattern}'; selecting the first one"
            )
        self.default_session_id, info = matched[0]
        _log(
            f"Successfully set the default session: {self.default_session_id}: {info['url']}"
        )
        return self.default_session_id

    def jump(self, url, timeout=15):
        return self.execute_js(f"window.location.href='{url}'", timeout=timeout)

    def back(self, timeout=15):
        return self.execute_js("window.history.back()", timeout=timeout)

    def forward(self, timeout=15):
        return self.execute_js("window.history.forward()", timeout=timeout)

    def reload(self, timeout=15):
        return self.execute_js("window.location.reload()", timeout=timeout)
    ...
    def newtab(self, url=None):
        ...
        return self.execute_js(
            _json.dumps({"cmd": "tabs", "method": "create", "url": url})
        )
    ...
    def close_tab(self, tab_id=None):
        ...
        result = self.execute_js(
            _json.dumps({"cmd": "tabs", "method": "remove", "tabId": int(tid)})
        )
