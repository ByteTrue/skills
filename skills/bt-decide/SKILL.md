...
> 共享路径与命名约定看 `.bytetrue/reference/shared-conventions.md`。产物写入 `.bytetrue/compound/`，命名 `YYYY-MM-DD-decision-{slug}.md`，frontmatter 带 `doc_type: decision`。
...
- 新建：写入 `.bytetrue/compound/YYYY-MM-DD-decision-{slug}.md`，frontmatter 顶部带 `doc_type: decision`
...
2. `.bytetrue/attention.md` 是否应追加一句启动必读摘要——`constraint` 或 `convention` 通常应该
...
python .bytetrue/tools/search-yaml.py --dir .bytetrue/compound --filter doc_type=decision --query "{关键词}" --json
```
...
4. **不主动修改 attention.md 和 ARCHITECTURE.md**——Phase 5 只提示，由用户决定；attention.md 追加交给 `bt-note`
