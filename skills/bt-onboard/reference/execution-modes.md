...
| `strict-evidence` | bugfix, regression-sensitive behavior, complex business logic, cross-boundary contract, security/data risk | use fresh evidence: root-cause-first for bugs, red-first where testable, and full verification before completion |
...
2. Upgrade to `strict-evidence` when failure cost is high or a bug/root-cause claim is involved.
...
- `bt-issue-fix`: uses `strict-evidence` for root-cause fixes and escalates to `break-loop` after repeated failed fixes.
