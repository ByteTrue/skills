...
| 阶段 | 子技能 | 主导 | 产出 |
|---|---|---|---|
| 1 问题报告 | `bt-issue-report` | 用户描述，AI 引导 | `{slug}-report.md` |
| 2 根因分析 | `bt-issue-analyze` | AI 读代码分析，用户确认 | `{slug}-analysis.md` |
| 3 修复验证 | `bt-issue-fix` | AI 按分析定点修复，用户验证 | 代码 + `{slug}-fix-note.md` + scoped-commit |
...
**判定口径**：是否进快速通道由 `bt-issue-report` 的启动检查做唯一正式判定。一旦进标准路径默认不再二次改判——避免三个阶段对路径各说各话。
...
进入本技能先 Glob `.bytetrue/issues/`，自己读已有文件才有数。

| 当前状态 | 触发哪个子技能 |
|---|---|
| 刚发现问题，没有任何文件 | `bt-issue-report`（那里判断走标准还是快速） |
...
- `.bytetrue/reference/shared-conventions.md` — 跨阶段共享口径
- `.bytetrue/attention.md` — ByteTrue 启动注意事项和项目硬约束
- `.bytetrue/architecture/ARCHITECTURE.md` — 根因分析时可能要查
