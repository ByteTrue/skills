...
Usage examples:
  # Search .bytetrue/compound (learning / trick / decision / explore docs share this dir)
  python .bytetrue/tools/search-yaml.py --dir .bytetrue/compound --filter doc_type=learning --filter track=pitfall
  python .bytetrue/tools/search-yaml.py --dir .bytetrue/compound --filter "doc_type=decision|explore|learning"
  python .bytetrue/tools/search-yaml.py --dir .bytetrue/compound --filter doc_type=trick --filter tags~=prisma
  ...
  python .bytetrue/tools/search-yaml.py --dir .bytetrue/compound --filter doc_type=learning --filter track=knowledge --json

  # Sort by a frontmatter date field (works on any ISO-8601 date string, YAML date, or sortable value)
  python .bytetrue/tools/search-yaml.py --dir .bytetrue/library-docs --sort-by last_reviewed --order asc   # oldest first (stalest)
