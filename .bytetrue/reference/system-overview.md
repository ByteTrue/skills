...
- `bt-req` — 起草或刷新 `.bytetrue/requirements/` 下的需求文档——系统的能力愿景层，当前/历史/未来边界由字段表达，status 使用统一词表
- `bt-arch` — 架构相关一站式:起草新架构文档 / 刷新已有文档 / 做架构体检(含 design 自洽 / design↔代码一致 / architecture 目录多份文档间一致)。architecture 只记现状
- `bt-audit` — 在用户限定的代码范围内主动扫描 bug 风险、安全问题、性能问题、可维护性债务和架构偏离；只产出 findings，不直接修复
...
| 体检系统 / 扫 bug 风险 / 审计代码 / 想知道哪里可优化 | `bt-audit`（产出 findings list，再把修复建议路由到 issue / refactor / architecture / grill） |
...
- **愿景档案**(requirements)——描述"用户需要什么、系统提供什么能力来满足"。`status` 使用统一五态词表；未来/当前/过期等语义用 `current` / `validity` 等字段表达。draft req 可独立于实现存在——先把愿景定下来，后续 roadmap 排期和 design 实现才有稳定对齐基准
- **结构档案**(architecture)——描述"系统现在用什么结构实现"。只记现状,默认在 feature-acceptance 时跟着代码同步;必要时由 bt-arch 主动刷新。**不写"未来会加什么层"**
- **规划档案**(roadmap)——描述"接下来打算怎么分步实现"。独立于愿景和结构档案,改动不牵连 requirements / architecture。所有条目 done / dropped 后 roadmap 进入 `status: done`,作为历史档案留存
- **单次动作**(feature / issue / refactor / audit)——本次要做的一件具体事情的 spec 或 findings。动作走完后,相关沉淀提炼进愿景档案、结构档案、后续工作流和沉淀类文档
...
- `.bytetrue/reference/config.md` — `.bytetrue/config.yaml` 字段语义
- `.bytetrue/reference/config.schema.yaml` — `.bytetrue/config.yaml` 机器可读 schema 与 onboard 默认种子
- `.bytetrue/reference/execution-modes.md` — light / standard / strict-evidence / break-loop 四档工作流轻重与证据纪律
- `.bytetrue/reference/implementation-review.md` — implementation 完成后、acceptance 前的规格符合度与代码质量 review gate
- `.bytetrue/reference/context-manifest.md` — feature-local implement/check JSONL read-set contract
- `.bytetrue/reference/subagent-handoff.md` — implement / check / research 子代理或内联角色的 handoff prompt contract
- `.bytetrue/reference/research-first.md` — 技术选择、外部 API / workflow 等事实会改变方向时，先用 `bt-explore spike` 留证据再决策
- `.bytetrue/reference/worklog-report-feed.md` — 轻量工作记录 feed，用于周报、handoff、恢复背景和 AI 工作审计
- `.bytetrue/reference/maintainer-notes.md` — 断点恢复、新增子工作流的登记

目录结构(config.yaml、requirements/、architecture/、roadmap/、features/、issues/、refactors/、audits/、compound/、brainstorms/、worklog/、tools/、reference/)的权威定义在 `shared-conventions.md`。要改目录先改那里——方法是改 `bt-onboard/reference/shared-conventions.md` 这个模板,新项目 onboard 时会带上新版本。
