...
维护 `skills/bt-onboard/reference/` 时，acceptance 前要和当前项目 `.bytetrue/reference/` 做一次对照：

- **managed identical**：共享 contract 文件应当 current / onboard 完全一致，例如 execution modes、implementation review、context manifest、subagent handoff、research-first、workflow-state breadcrumb、worklog report-feed。
- **managed localized/current-specific**：同一 contract 可以因项目可读性不同而不完全相同，例如当前中文 system overview 与 onboard 英文模板。
- **project-owned**：`domain-context.md` 和 `project-management.md` 不得在未确认时覆盖。

文件名不一致通常是 drift；内容不一致只有落在 localized/current-specific 或 project-owned 分类时才可接受。