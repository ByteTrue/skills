...
- 提取 `checks`：第 1 节"明确不做"→ 范围守护；第 2.1 接口 → 名词契约；第 2.2 主流程 + 流程级约束 → 编排骨架；第 2.3 挂载点 → 挂载点；第 3 节场景清单 → 验收场景；第 3.2 Behavior Delta → `behavior-delta` 检查
...
**bt-roadmap 的职责**：生成和维护 roadmap 主文档 + items.yaml；把 `pending` 改 `dropped`（用户放弃时）；不改 `active` / `done`（feature 技能负责）。
...
1. **只增不删**——已归档除非被明确取代（`status: archived` + `superseded_by`）否则不删；理由丢失成本极高
...
5. **起草前先查重叠**——动手写前用 `search-yaml.py --query` 查语义相近的旧文档。命中就把候选列给用户在三条路径里选：
   ...
   - **supersede**：旧文档保留原文，`status: archived` + `superseded_by: {新文件名}`，正文顶部加 `**[已取代]** 见 {新 slug}`；新文档 frontmatter 带 `supersedes: {旧文件名}`
