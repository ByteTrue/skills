...
- ByteTrue 提供可选 runtime breadcrumb 层：Pi package 可加载 `extensions/bytetrue-breadcrumb.ts`，在每轮 agent 启动前从现有 `.bytetrue` artifacts 派生当前阶段、活动产物、下一步和 guardrails 并注入提示；它是 runtime aid，不是 source of truth，不创建 `.bytetrue/state`，也不是核心流程依赖。
