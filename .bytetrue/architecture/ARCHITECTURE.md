...
- ByteTrue 的 subagent handoff 是可选执行基础设施：父会话保留 scope、用户对齐、最终 synthesis 和 lifecycle transition；执行面按 native subagent → synchronous non-interactive child agent → inline role execution 逐级 fallback；任一子/内联角色都只执行 bounded `implement` / `check` / `research` 任务，并通过 Active Work Block + context manifest 获得上下文。
...
- ByteTrue 提供轻量 worklog/report-feed 作为次级工作记录层：`.bytetrue/worklog/` 可按月记录工作区间、关联 artifacts、验证、状态和下一步，用于周报、handoff、恢复背景和 AI 工作审计；正式 feature / issue / roadmap / requirements / architecture / compound 仍是事实源，worklog 不保存原始聊天记录。
- ByteTrue 的 onboard 模板是新项目获得共享 workflow contract 的发布面：`skills/bt-onboard/reference/` 维护 skill-package-managed references，当前项目 `.bytetrue/reference/` 可因本地化或 project-owned 配置存在有意差异；维护时用 reference parity audit 区分 managed identical、managed localized/current-specific 和 project-owned。
- ByteTrue 现在使用 skills-first control plane：`.bytetrue/config.yaml` 承载 workflow / tracker / dispatch / docs 的当前配置值；所有 workflow artifact 的 `status` 统一为 `pending` / `active` / `done` / `dropped` / `archived`；`bt` 和 `bt-feat` 通过最小 artifact 状态判断续接阶段；workflow skills 会消费 `workflow.mode: auto` / `tracker.sync_policy: auto_preview`，只在确定的低风险下一步自动继续或准备预览，并始终停在 `ask_before` / review 边界；开放式大讨论使用 `.bytetrue/brainstorms/`；稳定 Behavior Delta 写回 requirements 的 `Current Behavior` 或 architecture 的 `Observable Contract`；没有 hidden runtime state。
