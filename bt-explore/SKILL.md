---
name: bt-explore
description: 对仓库做定向代码探索并把"提问→读代码→得结论"沉淀为可检索证据，三种类型 question / module-overview / spike。触发：用户说"先 explore 一下"、"这个仓库里 X 怎么实现"、"快速熟悉这个模块"、"zoom out / 上升一层看整体"、"把探索结果存档"。
---
...
同一个问题第一次花两小时查代码，第二次应该五分钟内找到答案——前提是第一次做完留下证据化的记录。bt-explore 把"提问 → 读代码 → 得结论"沉淀成可检索的探索文档。
...
- 新人入仓快速理解模块边界 / 调用链 / 入口
- 用户说 "zoom out" / "上升一层看" / "我不熟这块，先讲整体关系" / "给我模块和调用方地图"
...
> 共享路径与命名约定看 `.bytetrue/reference/shared-conventions.md`。产物写入 `.bytetrue/compound/`，命名 `YYYY-MM-DD-explore-{slug}.md`，frontmatter 带 `doc_type: explore`。
...
| `module-overview` | 快速梳理某模块结构 / 边界 / 入口 / 依赖 / 调用方地图；承接 Matt `zoom-out` |
...
- `module-overview` 额外回答：该模块上游调用方是谁、下游依赖谁、public interface 是什么、哪些 seam / adapter 形成边界；只讲现状，不替用户设计目标态。
...
- 新建：写入 `.bytetrue/compound/YYYY-MM-DD-explore-{slug}.md`，frontmatter 带 `doc_type: explore`
...
python .bytetrue/tools/search-yaml.py --dir .bytetrue/compound --filter doc_type=explore --query "{关键词}" --json
```
