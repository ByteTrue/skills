...
ByteTrue 把开发活动建模成 **7 个实体 + 3 个流程**，所有产物聚在 `.bytetrue/`：
...
**横切**：方案还没问透先 `bt-grill`；需要把 confirmed ByteTrue 产物同步到 GitHub/GitLab/local tracker，或 triage 外部 incoming issues 时走 `bt-tracker`；流程跑完发现"值得记下来" → `bt-learn` / `bt-trick` / `bt-decide` / `bt-explore` 沉淀到 `compound/`。
